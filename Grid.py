...
class TttGrid:
    ...
    def update_grid(self, x, y, letter):
        self.row_list[y - 1] = (
            (self.row_list[y - 1])[: self.x_index(x)]
            + letter
            + (self.row_list[y - 1])[self.x_index(x) + 1 :]
        )
...
class AvailableMoves:
    ...
    def win(self):
        ...
        for col in range(3):
            if (
                self.available[0][col]
                == self.available[1][col]
                == self.available[2][col]
                and self.available[0][col] != 3
            ):
                return True

        # Check diagonals
        if (
            self.available[0][0] == self.available[1][1] == self.available[2][2]
            and self.available[0][0] != 3
        ):
            return True
        if (
            self.available[0][2] == self.available[1][1] == self.available[2][0]
            and self.available[0][2] != 3
        ):
            return True
